...
public class GymCustomerFlipFitMenu {
    ...
    private void bookSlotFlow(GymCustomer customer, String gymId, Scanner scanner) {
        ...
        List<com.flipfit.bean.Schedule> availableSchedules = new java.util.ArrayList<>();
                ...
                for (com.flipfit.bean.Slot slot : com.flipfit.business.GymOwnerServiceImpl.slotList) {
                    if (slot.getSlotId().equals(schedule.getSlotId()) && slot.getCentreId().equals(gymId)) {
                        if (schedule.getAvailableSeats() > 0) {
                            ...
                            System.out.println(availableSchedules.size() + ". Time: "
                                    + slot.getStartTime() + " (Seats: " + schedule.getAvailableSeats() + ")");
                        }
                    }
                }
            }
        ...
        if (availableSchedules.isEmpty()) {
            System.out.println("No slots available for this gym on this date.");
        } else {
            ...
            if (slotChoice > 0 && slotChoice <= availableSchedules.size()) {
                String scheduleId = availableSchedules.get(slotChoice - 1).getScheduleId();
                bookingService.addBooking(customer.getUserId(), scheduleId);
            } else {
